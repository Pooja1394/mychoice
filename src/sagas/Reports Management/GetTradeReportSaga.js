...
export function getData(data) {
  ...
  return axios({
    ...
    headers:headers,
    data:data,
    url: basepath+`reports/tradeDetails`,
    });
}
  
// worker saga: makes the api call when watcher saga sees the action
export function* GetTradeReportSaga(action) {
  try {
    ...
    yield put({ type:GET_TRADE_REPORT_SUCCESS, data });
   
    
  } catch (error) {
    // dispatch a failure action to the store with the error
    yield put({ type: GET_TRADE_REPORT_FAILURE, error })
