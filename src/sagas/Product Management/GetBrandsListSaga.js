...
export function getData(data) {
  ...
  return axios({
    ...
    headers:headers,
    data:data,
    url: basepath+`brands/getbrands`,
    });
}
  
// worker saga: makes the api call when watcher saga sees the action
export function* GetBrandsListSaga(action) {
  try {
    ...
    yield put({ type:GET_BRANDS_CALL_SUCCESS, data });
   
    
  } catch (error) {
    // dispatch a failure action to the store with the error
    yield put({ type: GET_BRANDS_CALL_FAILURE, error })
