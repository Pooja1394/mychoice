...
import {USER_TABLE_CSV,GET_DEPOSIT_REPORT_REQUEST} from "../../actions/types";
import {connect} from "react-redux";
import moment from "moment";
import {checkdisplay} from "../../utils/Method"
class DepositReports extends Component {
  constructor(){
    super();
    this.state={
  ...
  this.props.getDepositReportsData();
    
  }
    render() {
      const {fetching,getDepositReportsData} =this.props;
      let depositReportList = [];
      if (this.props.state.depositReportsData) {
        this.props.state.depositReportsData.finalResult.map((data, key) => {
          let obj;
          obj = {
            ...data,
            index: key + 1
        ...
        <Row>
            <Column header="S.NO" rowSpan={3} style={{width:"50px"}}/>
            <Column header="Date" rowSpan={3} />
            <Column header="No.of Users" rowSpan={3} />
            <Column header="Deposit" colSpan={2} />
            <Column header="Profit/Loss" rowSpan={2} />
        </Row>
        {/* <Row>
...
const mapDispatchToProps = dispatch => {
  return {
    getDepositReportsData: data =>
      dispatch({ type: GET_DEPOSIT_REPORT_REQUEST, data }),
    onRequestExportCSV: (data, value) => dispatch({ type: USER_TABLE_CSV, data, value })
  };
};
export default connect(  mapStateToProps,  mapDispatchToProps)(DepositReports);
